...
async function summarizeConversation(convo, callerPhoneNumber, tradie) {
  ...
  - reminderLocation: Location (lat, long) of the reminder if the user is setting a reminder and mentioned a location if not set it to null. Set the mentioned location's lat long according to country ${contactInfo?.countryCode || 'unknown'}
  - reminderTime: Time of the reminder if the user is setting a reminder and mentioned date or a time if not set it to null. Its type is dateTime. Date should be current date if not mentioned else the mentioned date. It cannot be in the past. Today is ${new Date().toString()}. Also convert the mentioned time into utc using the timezone ${contactInfo?.timezone || 'UTC'}
